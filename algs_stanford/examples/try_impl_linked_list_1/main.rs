/// Safe but not efficient stack queue
...
///
/// Ref: https://rust-unofficial.github.io/too-many-lists/third-final.html
mod link {
    pub struct Node {
        s: String,
    ...
    impl LinkedList {
        ...
        pub fn pop_tail(&mut self) -> Option<String> {
            ...
            while let Some(ref mut next_before) = one_before_tail.next {
                if let Some(ref mut _next) = next_before.next {
                    one_before_tail = next_before;
                } else {
                    break;
                }
            }
        ...
        pub fn push(&mut self, s: String) -> &mut LinkedList {
            ...
            self.head.link(new_head);
            self
        }

        /// pop element from head
        pub fn pop(&mut self) -> Option<String> {
            if let Some(first) = self.head.next.take() {
...
fn main() {
    ...
    println!("pop tail: {}", list.pop_tail().unwrap_or("?".to_string()));
    println!("pop tail: {}", list.pop_tail().unwrap_or("?".to_string()));
